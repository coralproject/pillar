...
// 	"github.com/buth/pillar/pkg/backend"
// 	"github.com/buth/pillar/pkg/backend/mongodb"
//
...
// 		log.Dev(uid, "buildTimeseries", "Collection %v is empty", c)
// 	}
//
...
//
//
// 	log.User(uid, "buildTimeseries", "Building timeseries: collections %v, durations %v", cs, ds)
//
// 	limiter := 0
// 	limit := 10000
//
...
// 		for dk, d := range ds {
//
// 			log.Dev(uid, "buildTimeseries", "Building %v from %v", k, c)
//
// 			start, end := getRange(c)
// 			log.Dev(uid, "buildTimeseries", "From %+v to %+v", start, end)
//
...
// 				if err != nil {
// 					log.Error(uid, "Build", err, "Error inserting %+v\n", timeslice)
// 				}
//
// 				fmt.Printf("%v %+v\n", limiter, timeslice)
//
